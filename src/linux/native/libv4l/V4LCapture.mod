						      ...
						      jobject jvpict)
{
	...
	printf("setPicture() failed - depth=%d, palette=%d\n",
						     ...
						     jfloat jrate)
{
    ...
    vpict.depth = jdepth;
    printf("setFormat - depth=%d, palette=%d\n",
	   vpict.depth, vpict.palette);
    if (ioctl(capture->fd, VIDIOCSPICT, &vpict) < 0) {
	return -1;
    }

    ioctl(capture->fd, VIDIOCGWIN, &vid_win);
    
    vid_win.x = 0;
    vid_win.y = 0;
    vid_win.width = jwidth;
    vid_win.height = jheight;
    if (ioctl(capture->fd, VIDIOCSWIN, &vid_win) == -1) {
	printf("Error setting size VIDIOCSWIN\n");
	//return -1;
    }
						 ...
						 jobject javapeer)
{
    ...
    if (capture->grabMethod == GRAB_METHOD_READ || ioctl(capture->fd, VIDIOCGMBUF, &(capture->mbuf)) < 0) {
	...
	for (i = 0; i < capture->mbuf.frames; i++) {
		...
		printf("VIDIOCMCAPTURE failed in start\n");
		return -1;
	    }
	}
						...
						jobject javapeer)
{
	...
	/* Free all the frames */
	for (i = 0; i < capture->mbuf.frames; i++) {
	    result = ioctl(capture->fd, VIDIOCSYNC, &i);
	    printf("stop.sync = %d\n", result);
	}
	/* unmap the memory */
