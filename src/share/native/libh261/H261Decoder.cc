...
#include "com_sun_media_codec_video_h261_NativeDecoder.h"	      //
#include "jni-util.h"


/*************************************************************
 * Class for calling the decoder and renderer.
 */
class H261Decoder {
  public:
    H261Decoder(u_int w, u_int h);
    ~H261Decoder();
    int handlePacket(char *data, int dataLen, unsigned long flags,
		     unsigned long syncBit, char *outBuf,
		      int width, int height,
		     int sizeChgd);
    void resize(int, int);
    int  getPictureOffset();
    int formatChange;
    void *picture;
  private:
    P64Decoder *codec_;
...
hufftab	P64Decoder::ht_mba_;
hufftab	P64Decoder::ht_mvd_;
hufftab	P64Decoder::ht_cbp_;
hufftab	P64Decoder::ht_tcoeff_;
hufftab	P64Decoder::ht_mtype_;

/***********************************************************
 * Native code implementation for the h261_Codec.java class
 */

#define JAVA_STRUCT_STRING              "nativeData"

JNIEXPORT jboolean JNICALL
Java_com_sun_media_codec_video_h261_NativeDecoder_initNativeDecoder(JNIEnv *env,
								    jobject jdecoder)
{
    unsigned int width = (unsigned int) GetIntField(env, jdecoder, "width");
    unsigned int height = (unsigned int) GetIntField(env, jdecoder, "height");
    H261Decoder *decoder = new H261Decoder(width, height);
    decoder->picture = NULL;
    SetIntField(env, jdecoder, JAVA_STRUCT_STRING, (int) decoder);
    return 1;
}

JNIEXPORT void JNICALL
Java_com_sun_media_codec_video_h261_NativeDecoder_allocatePicture(JNIEnv *env,
								  jobject jdecoder,
								  jint size)
{
    H261Decoder * decoder = (H261Decoder*)
                             GetIntField(env, jdecoder, JAVA_STRUCT_STRING);
    if (decoder == NULL)
	return;
    if (decoder->picture)
	free(decoder->picture);
    else
	decoder->picture = (void *) malloc(size);
}

JNIEXPORT jboolean JNICALL
Java_com_sun_media_codec_video_h261_NativeDecoder_freeNativeDecoder(JNIEnv *env,
								    jobject jdecoder)
{
    H261Decoder * decoder = (H261Decoder*)
                             GetIntField(env, jdecoder, JAVA_STRUCT_STRING);
	...
	free(decoder->picture);
    delete decoder;
    SetIntField(env, jdecoder, JAVA_STRUCT_STRING, (int) 0);
    return 1;
}


JNIEXPORT jboolean JNICALL
Java_com_sun_media_codec_video_h261_NativeDecoder_decodeNative(JNIEnv *env,
							       jobject jdecoder,
							       jobject inputBuffer,
							       jobject outputBuffer,
							       jlong outBytes)
{
    H261Decoder *h261Decoder = (H261Decoder*)
	                         GetIntField(env, jdecoder, JAVA_STRUCT_STRING);

    jarray inArr = (jarray) GetObjectField(env, inputBuffer, "data", "Ljava/lang/Object;");
    jarray outArr = NULL;

    unsigned int flags = (unsigned int) GetIntField(env, jdecoder, "h261Flags");
    unsigned int skipBytes = (unsigned int) GetIntField(env, jdecoder, "skipBytes");
    int syncBit = (int) GetIntField(env, jdecoder, "syncBit");
    int dataLen = (int) GetIntField(env, inputBuffer, "length");
    int sizeChanged = (int) GetIntField(env, jdecoder, "sizeChanged");

    char *inBuf   = (char*) env->GetByteArrayElements((jbyteArray) inArr, 0);
    char *picture = (char*) h261Decoder->picture;

    int width  = GetIntField(env, jdecoder, "width");
    int height = GetIntField(env, jdecoder, "height");

    int offset = (int) GetIntField(env, inputBuffer, "offset");
    int returnVal;

    if (h261Decoder->handlePacket(inBuf+offset+skipBytes, dataLen-skipBytes, flags, syncBit,
				  picture, width, height, sizeChanged)) {
	
	int size = (width * height) + ((width * height) >> 1);
	if (outBytes == 0) {
	    outArr = (jarray) GetObjectField(env, outputBuffer,
					     "data", "Ljava/lang/Object;");
	    env->SetByteArrayRegion((jbyteArray) outArr, 0, size,
				    (jbyte*) picture + h261Decoder->getPictureOffset());
	} else {
	    memcpy((void*)outBytes,
		   picture + h261Decoder->getPictureOffset(), size);
	}
    ...
    env->ReleaseByteArrayElements((jbyteArray) inArr, (signed char *) inBuf, JNI_ABORT);
    return returnVal;
}
...
H261Decoder::H261Decoder(u_int w, u_int h)
{
    codec_ = NULL; // We dont know the codec type yet.
    decimation_ = 411;
    now_ = 1;
    nblk_ = 0;
    ndblk_ = 0;
...
int
H261Decoder::handlePacket(char *data, int dataLen, unsigned long flags,
			  unsigned long syncBit,
			  char *outBuf,
			  int width, int height, int sizeChanged)
{
    formatChange = 0;
    // Create the codec
    if (codec_ == NULL || codec_->width() != width) {
	u_int fmt;
	if (width == 352)
	    fmt = IT_CIF;
	else
	...
	if (((flags >> 25) & 0x1) != 0) {
	    codec_ = new IntraP64Decoder(fmt);
	} else {
	    codec_ = new FullP64Decoder(fmt);
	}
	codec_->marks(rvts_);
    }
    unsigned int sbit = (flags >> 29) & 7;
    unsigned int ebit = (flags >> 26) & 7;
    unsigned int quant = (flags >> 10) & 0x1f;
    unsigned int mvdh = (flags >> 5) & 0x1f;
    unsigned int mvdv = flags & 0x1f;
    unsigned int mba, gob;

    //if (h261_rtp_bug_) { 	-ivg
    if (0) {
	mba = (flags >> 19) & 0x1f;
	gob = (flags >> 15) & 0xf;
	if (gob > codec_->ngob_) {
    ...
    codec_->mark(1);
    int oldFormat = codec_->getFormat();
//    *nBlocks = 0;
    if (sizeChanged) {
	codec_->theFrame_ = 0;
	codec_->theOtherFrame_ = 0;
    }
    codec_->decode((u_char*)data, dataLen, (u_char*)outBuf,
		   sbit, ebit, mba, gob, quant,
		   mvdh, mvdv);
    if (codec_->getFormat() != oldFormat) {
	formatChange = 1;
	// return 0;
    }

    /*
    if (codec_->width() != sourceWidth_)
	resize(codec_->width(), codec_->height());
    */
    /*
    scale = 1;
    if (codec_->getFormat() == IT_QCIF)
	scale = 2;
    */
    //printf("SyncBit = %d\n", syncBit);
    ...
    if (syncBit == 1) {
	u_char *ts = rvts_;
        int frameOffset = codec_->theFrame_;
        codec_->sync();
        ndblk_ = codec_->ndblk();
...
void
H261Decoder::resize(int width, int height)
{
    // timeval t = {0L, 0L};
    sourceWidth_ = width;
    sourceHeight_ = height;
    // nblk = no. of blocks in the frame. Block = 8x8
...
int
H261Decoder::getPictureOffset()
{
//  return codec_->theFrame_;

  // the offsets are swapped before we copy the picture to the output buffer
  return codec_-> theOtherFrame_ ;
}
...
void P64Decoder::init(u_int fmt)
{
    fmt_ = fmt;
    if (fmt_ == IT_CIF) {
	ngob_ = 12;
...
{
	ht_mtype_.prefix = (short *) htd_mtype;
	ht_mtype_.maxlen = htd_mtype_width;
	ht_mba_.prefix = (short *) htd_mba;
	ht_mba_.maxlen = htd_mba_width;
	ht_mvd_.prefix = (short *) htd_dvm;
	ht_mvd_.maxlen = htd_dvm_width;
	ht_cbp_.prefix = (short *) htd_cbp;
	ht_cbp_.maxlen = htd_cbp_width;
	ht_tcoeff_.prefix = (short *) htd_tcoeff;
 ...
 * at the front).
 */
int P64Decoder::decode(u_char* bp, int cc, u_char* yuvData,
		       int sbit, int ebit,
		       int mba, int gob, int mq, int mvdh, int mvdv)
{
	ps_ = (u_short*)bp;
	yuvBuffer_ = yuvData;
	front_ = yuvData + theFrame_;
	back_ = yuvData + theOtherFrame_;
	/*
...
FullP64Decoder::FullP64Decoder(u_int fmt)
{
    init(fmt);
}
...
{
    theFrame_ = 0;
    theOtherFrame_ = size_ + (size_ >> 1);
    front_ = yuvBuffer_ + theFrame_;
    back_ = yuvBuffer_ + theOtherFrame_;
}
...
{
    unsigned int p = theFrame_;
    theFrame_ = theOtherFrame_;
    theOtherFrame_ = p;
}
...
/**
 * Synchronizes the two buffers... Copies the changed macroblocks from
 * the back buffer to the front buffer and marks those blocks as OLD
 */
...
IntraP64Decoder::IntraP64Decoder(u_int fmt)
{
    init(fmt);
}
